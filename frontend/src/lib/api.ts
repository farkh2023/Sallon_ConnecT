...
export function handleApiError(error: unknown): string {
  if (error instanceof Error) return maskSensitiveClientText(error.message);
  if (typeof error === 'string') return maskSensitiveClientText(error);
...
async function request<T>(method: string, path: string, body?: unknown): Promise<T> {
  ...
  if (!res.ok) {
    const err = (await res.json().catch(() => ({} as Record<string, unknown>))) as Record<
      string,
      unknown
    >;
